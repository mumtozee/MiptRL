...
class FireReset(Wrapper):
    ...
    def __init__(self, env):
        ...
        if len(action_meanings) < 3:
            raise ValueError("env.unwrapped.get_action_meanings() must be of length >= 3 but is of length {len(action_meanings)}")
        if env.unwrapped.get_action_meanings()[1] != "FIRE":
            raise ValueError("env.unwrapped.get_action_meanings() must have 'FIRE' under index 1, but is {action_meanings}")
...
class StartWithRandomActions(Wrapper):
    ...
    def reset(self, **kwargs):
        obs, info = self.env.reset(**kwargs)
        if self.real_done:
            num_random_actions = self.unwrapped.np_random.integers(low=1, high=self.max_random_actions + 1)
...
class ImagePreprocessing(ObservationWrapper):
    ...
    def __init__(self, env, height=84, width=84, grayscale=True):
        ...
        if self.grayscale:
            self.observation_space = Box(low=low, high=high, shape=(height, width), dtype=dtype)
        else:
            self.observation_space = Box(low=low, high=high, shape=(height, width, *self.observation_space.shape[2:]), dtype=dtype)
...
class QueueFrames(ObservationWrapper):
    ...
    def __init__(self, env, nframes, concat=False):
        ...
        self.observation_space = Box(ospace.low.min(), ospace.high.max(), oshape, ospace.dtype)

    def observation(self, observation):
        self.obs_queue.append(observation)
        return (np.concatenate(self.obs_queue, -1) if self.concat else np.dstack(self.obs_queue))
...
class SwapImageAxes(ObservationWrapper):
    ...
    def __init__(self, env):
        ...
        self.observation_space = Box(low=0.0, high=1.0, shape=(old_shape[-1], old_shape[0], old_shape[1]), dtype=np.float32)
...
def get_summaries_class(summaries):
    summaries_class_map = {"Numpy": NumpySummaries, "Tensorboard": TensorboardSummaries}
    if summaries in summaries_class_map:
        return summaries_class_map[summaries]

    raise NotImplementedError(f"Unknown summaries: {summaries}. Supported summaries: {summaries_class_map.keys()}")
...
class _thunk:
    ...
    def __call__(self):
        return nature_dqn_env(self.env_id, summaries=False, clip_reward=False, **self.kwargs)
...
def nature_dqn_env(env_id, nenvs=None, seed=None, summaries="Numpy", clip_reward=True):
    ...
    if nenvs is not None:
        ...
        if isinstance(seed, int):
            seed = [seed] * nenvs
        if len(seed) != nenvs:
            raise ValueError(f"seed has length {len(seed)} but must have length equal to nenvs which is {nenvs}")
